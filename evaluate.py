...
Original file is located at
    https://colab.research.google.com/drive/1x4e_j40zXeXxOOpa5OW3fOASUdfx2Qll
"""
...
def align_translations(
    ...
    print(f"  ✓ Loaded {len(ref_df)} reference translations")
    ...
    print(f"  ✓ Loaded {len(hyp_df)} generated translations")
    ...
    print(f"  ✓ Aligned {len(merged_df)} examples")

    # Check for missing files
    missing = len(ref_df) - len(merged_df)
    if missing > 0:
        print(f"  ⚠️  {missing} audio files from reference not found in hypothesis")
    ...
    if nan_count > 0:
        ...
        print(f"  ✓ {len(result_df)} valid examples remaining")
...
def calculate_meteor(hypotheses: List[str], references: List[str]) -> float:
    ...
    # Report errors
    if errors > 0:
        print(f"    ⚠️  {errors} examples skipped due to errors")
        if error_details:
    ...
    if scores:
        ...
        print(f"    ✓ Successfully calculated for {len(scores)}/{len(hypotheses)} examples")
        return round(avg_score, 2)
    else:
        print(f"    ⚠️  No METEOR scores calculated!")
        return 0.0
...
def evaluate_translation(
    ...
    try:
        ...
        print(f"  ✓ BLEU: {bleu_metrics['BLEU']}")
    except Exception as e:
        print(f"  ⚠️  Error: {e}")
    ...
    if 'BLEU' in metrics:
        status = '🟢 Excellent' if metrics['BLEU'] > 40 else '🟡 Good' if metrics['BLEU'] > 30 else '🔴 Needs improvement'
        print(f"  BLEU:   {metrics['BLEU']}/100  → {status}")

    if 'METEOR' in metrics:
        status = '🟢 Excellent' if metrics['METEOR'] > 60 else '🟡 Good' if metrics['METEOR'] > 45 else '🔴 Needs improvement'
        print(f"  METEOR: {metrics['METEOR']}/100 → {status}")

    if 'WER' in metrics:
        status = '🟢 Excellent' if metrics['WER'] < 30 else '🟡 Good' if metrics['WER'] < 50 else '🔴 Needs improvement'
        print(f"  WER:    {metrics['WER']}%      → {status}")

    print("\n💡 What these numbers mean:")
    print("  • BLEU measures n-gram overlap (0-100, higher is better)")
    print("  • METEOR measures semantic similarity (0-100, higher is better)")
    print("  • WER measures word error rate (0-100, lower is better)")
