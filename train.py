...
Original file is located at
    https://colab.research.google.com/drive/1pBYIvaUEpATsCcjSYIInCeq3kjfZxaKj
"""
...
Dataset:
    - Južne Vesti (train, validation, test splits)
"""
    ...
    for path in possible_paths:
        if os.path.exists(path):
            ...
            if any(f.endswith('.wav') for f in files):
                print(f"  ✅ Found audio files in: {path}")
                return path
...
def load_juzneves_data(txt_file: str, audio_folder: str) -> Dataset:
    """
    Load Južne Vesti dataset from TSV file.
    ...
    for idx, row in df.iterrows():
        ...
        if os.path.exists(audio_path):
            ...
            missing_count += 1
            if missing_count <= 5:
                print(f"  ⚠️  File not found: {row['audio_file']}")

    if missing_count > 5:
        print(f"  ⚠️  ... and {missing_count - 5} more files missing")

    print(f"  ✅ Loaded {len(audio_paths)}/{len(df)} audio files")
...
def compute_metrics(pred, processor, wer_metric):
    ...
    if isinstance(pred_ids, np.ndarray):
        ...
        if invalid_mask.any():
            ...
            print(f"⚠️  Found {num_invalid} invalid token IDs")
...
if __name__ == "__main__":
    ...
    print("=" * 70)
    print("Serbian → English Speech Translation")
    ...
    print(f"\n📦 Loading SeamlessM4T processor...")
    processor = SeamlessM4TProcessor.from_pretrained(MODEL_NAME)

    print(f"📦 Loading SeamlessM4T model in 8-bit...")
